...
function startProxy (aReqHandlersArr) {
  ...
  if (global.homeMode) {
    ...
    console.log(('HTTP proxy listening on ' + gc.proxySvr1).yellow);
    httpProxy.createServer(function (req, res, proxy) {
      var uri = url.parse(req.url);
      ...
      if (proxies.exec(uri.path)) {
        ...
        proxy.proxyRequest(req, res, {
          host : gc.apiHost, // app dot what ever the external server is
          port : gc.api // port 80
        });
      }
      else {
...
fiapp.use(flatiron.plugins.cli, {
  usage: [
    ...
    'devtools',
    '',
    'Starts a local webserver serves static files',
    '',
    'Usage:',
...
// default mode. use this mode when you have access to the API and when you are
